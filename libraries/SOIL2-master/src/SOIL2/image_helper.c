	...
	)
{
	...
	*/
    dx = (_width - 1.0f) / (resampled_width - 1.0f);
    dy = (_height - 1.0f) / (resampled_height - 1.0f);
    for ( y = 0; y < resampled_height; ++y )
    {
		...
		if( inty > _height - 2 ) { inty = _height - 2; }
		sampley -= inty;
        for ( x = 0; x < resampled_width; ++x )
        {
			...
			if( intx > _width - 2 ) { intx = _width - 2; }
			samplex -= intx;
			/*	base index into the original image	*/
			base_index = (inty * _width + intx) * channels;
            for ( c = 0; c < channels; ++c )
            {
				...
				value += orig[base_index+_width*channels]
							*(1.0f-samplex)*(sampley);
				value += orig[base_index+_width*channels+channels]
							*(samplex)*(sampley);
	...
	)
{
	...
	mip_width = _width / block_size_x;
	mip_height = _height / block_size_y;
	if( mip_width < 1 )
	{
		mip_width = 1;
	}
	...
	for( j = 0; j < mip_height; ++j )
	{
		for( i = 0; i < mip_width; ++i )
		{
			for( c = 0; c < channels; ++c )
			{
				const int index = (j*block_size_y)*_width*channels + (i*block_size_x)*channels + c;
				int sum_value;
				int u,v;
				...
				if( block_size_x * (i+1) > _width )
				{
					u_block = _width - i*block_size_y;
				}
				if( block_size_y * (j+1) > _height )
				{
					v_block = _height - j*block_size_y;
				}
				...
				for( u = 0; u < u_block; ++u )
				{
					sum_value += orig[index + v*_width*channels + u*channels];
				}
	...
	)
{
	...
	for( i = 0; i < _width*_height*channels; i += channels )
	{
		for( j = 0; j < nc; ++j )
		{
	...
	)
{
	...
	if( channels == 3 )
	{
		for( i = 0; i < _width*_height*3; i += 3 )
		{
			int r = orig[i+0];
			int g = (orig[i+1] + 1) >> 1;
			int b = orig[i+2];
...
)
{
	...
	if( rescale_to_max )
	{
		scale = 255.0f / find_max_RGBE( image, _width, _height );
	}
	for( i = _width * _height; i > 0; --i )
	{
...
)
{
	...
	if( rescale_to_max )
	{
		scale = 255.0f * 255.0f / find_max_RGBE( image, _width, _height );
	}
	for( i = _width * _height; i > 0; --i )
	{
		/* decode this pixel, and find the max */
